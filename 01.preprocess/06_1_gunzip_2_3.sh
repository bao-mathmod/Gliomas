...
process_one() {
  local RUN="$1"
  local SRC_DIR="/mnt/12T/chibao/data/official_data/fastq_by_tech/bulk_or_plate_RNA/official_result/PRJNA683876"
  local DEST_BASE="/mnt/12T/chibao/data/official_data/fastq_by_tech/bulk_or_plate_RNA/official_cell/PRJNA683876"
  local DEST_DIR="${DEST_BASE}/${RUN}"
  mkdir -p "${DEST_DIR}"

  # R1
  pigz -p 7 "${SRC_DIR}/${RUN}_2.fastq"
  mv "${SRC_DIR}/${RUN}_2.fastq.gz" "${DEST_DIR}/${RUN}_S1_L001_R1_001.fastq.gz"

  # R2
  pigz -p 7 "${SRC_DIR}/${RUN}_3.fastq"
  mv "${SRC_DIR}/${RUN}_3.fastq.gz" "${DEST_DIR}/${RUN}_S1_L001_R2_001.fastq.gz"
}

export -f process_one

# Kick off two runs in parallel:
bash -lc 'process_one SRR13228579' &
bash -lc 'process_one SRR13228580' &
bash -lc 'process_one SRR13228581' &
bash -lc 'process_one SRR13228582' &
bash -lc 'process_one SRR13228583' &
bash -lc 'process_one SRR13228584' &
bash -lc 'process_one SRR13228585' &
