...
# # USER SETTINGS
# # ==============================================================================
# sample_id <- "Glioma_General"
# base_dir <- "/mnt/18T/chibao/gliomas/data_official/00_raw_data_adult/03_integrated/02_rpca/orig.ident/obj"
# out <- '/mnt/18T/chibao/gliomas/data_official/stuff/test_anno'
# # Input: The integrated myeloid Seurat object
# in_path <- file.path(base_dir, "rpca_integrated_orig.ident.rds")
...
# p1 <- DimPlot(seu, reduction = umap_reduction, group.by = "SingleR_Main", label = TRUE, repel = TRUE) + 
...
# p2 <- DimPlot(seu, reduction = umap_reduction, group.by = "SingleR_Fine", label = TRUE, repel = TRUE, label.size = 3) + 
...
# p3 <- FeaturePlot(seu, features = c("Microglia_Score", "TAM_Score"), reduction = umap_reduction, ncol = 2) & 
...
out_dir <- file.path(out, "03_Annotation_outputs_Azimuth", sample_id)
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
...
p_c1 <- DimPlot(seu, reduction = "umap.rpca", group.by = "predicted.class", label = TRUE, label.size = 4, repel = TRUE) +
...
p_c2 <- DimPlot(seu, reduction = "umap.rpca", group.by = "predicted.subclass", label = TRUE, label.size = 3, repel = TRUE) +
  ggtitle(paste0(sample_id, " Subclass")) + NoLegend()
