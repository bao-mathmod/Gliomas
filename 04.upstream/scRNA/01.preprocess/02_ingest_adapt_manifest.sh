...
apply_adaptive_scrna_filters_glioma <- function(sobj, mad_k=4, min_cells_after_filter=20) {
  ...
  # Safety bounds: Ensure the threshold never drops below 10% (protects high-quality batches) 
  # and never exceeds 40% (prevents keeping pure noise in terrible batches).
  mito_final  <- min(max(mito_upper, 10.0), 40.0) 

  # 2. Feature Filtering: Lower bound only. 
  # Upper bounds for nFeature_RNA and nCount_RNA are intentionally removed to preserve 
  # highly active/multinucleated glioma cells. Doublets are handled downstream by scDblFinder.
  passing_cells <- which(
  ...
  log(sprintf("QC thresholds for %s: nFeature_RNA > 200 | percent.mt < %.2f",
              sobj$sample_uid[1], mito_final))
...
for (i in seq_len(nrow(manifest))) {
  ...
  tryCatch({
    ...
    retention_rate <- (postqc_n / raw_n) * 100

    log(sprintf("Saving RDS for %s. Raw: %d, Post-QC: %d (%.1f%% retained)", 
            suid, raw_n, postqc_n, retention_rate))
    ...
    summary_df <- data.frame(
      ...
      retention_pct = round(retention_rate, 2), # NEW COLUMN
